...
public function main() {
    ...
    // Declares the queue.
    var queueResult = newChannel->queueDeclare(queueConfig = { queueName: "testQueue2" });
    ...
    var sendResult = newChannel->basicPublish("Hello from Ballerina", "testQueue2");
    if (sendResult is error) {
        io:println("An error occurred while sending the message");
    }

    boolean isClosedBefore = connection.isClosed();
    io:println(isClosedBefore);
    var closeResult = connection.abortConnection();
    if (closeResult is error) {
        io:println("Error occurred while closing the connection");
    }
    // Publishes the message using the routing key named "testQueue".
    var sendResult2 = newChannel2->basicPublish("Hello from Ballerina", "testQueue2");
    if (sendResult is error) {
        io:println("An error occurred while sending the message");
    }
